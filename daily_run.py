...
print(f"🔑 Clé API Tennis : {API_TENNIS_KEY[:5]}...")  # Vérification que la clé est bien injectée
print(f"📨 Chat ID : {CHAT_ID} - Token Telegram début : {TELEGRAM_TOKEN[:5]}...")

# 📦 Fonction modulaire pour les cotes
def get_odds():
    return build_odds_dataframe()

# 📡 Récupérer les matchs du jour via API-Tennis
def get_matches():
...
def run_prediction_and_send_message():
    try:
        ...
        send_telegram(f"❌ Erreur lors de la récupération des matchs : {e}")
        return
